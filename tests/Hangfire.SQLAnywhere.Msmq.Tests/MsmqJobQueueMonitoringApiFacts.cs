...
namespace Hangfire.Msmq.Tests
{
    public class MsmqJobQueueMonitoringApiFacts
    {
        ...
        {
            MsmqUtils.EnqueueJobId(null, "my-queue", "1");
            MsmqUtils.EnqueueJobId(null, "my-queue", "2");
            MsmqUtils.EnqueueJobId(null, "another-queue", "3");
        ...
        {
            for (var i = 1; i <= 10; i++) { MsmqUtils.EnqueueJobId(null, "my-queue", i.ToString()); }
            var api = CreateMonitoringApi();
